...
window.Cart = {
  ...
  open: function () {
    $('#cart-overlay, #cart-drawer').addClass('open');
    $('body').css('overflow', 'hidden');
    this.renderDrawer();
  },

  close: function () {
    $('#cart-overlay, #cart-drawer').removeClass('open');
    $('body').css('overflow', '');
  },
};
...
function loadProducts(page = 1, append = false) {
  ...
  const url = currentSearch
    ? `/api/products/search?q=${encodeURIComponent(currentSearch)}&page=${page}&limit=12`
        ...
        <div class="product-card" data-id="${p.id}">
          <div class="product-card-image-wrap">
            ...
            <div class="product-card-overlay">
              <button class="btn btn-primary btn-sm w-full quick-view-btn" data-id="${p.id}" ${p.stock === 0 ? 'disabled' : ''}>
                Quick View
              </button>
            </div>
          </div>
...
function renderPagination(pagination) {
  ...
  html += `<button class="pagination-btn" ${p.page <= 1 ? 'disabled' : ''} data-page="${p.page - 1}">&#8249;</button>`;
...
function openProductModal(productId) {
  ...
  $.get(`/api/products/${productId}`, function (data) {
    ...
    let imagesHtml = '';
    if (images.length > 0) {
      imagesHtml = `<div class="product-images-grid">`;
      images.forEach((img, i) => {
        imagesHtml += `<img src="${img.image_path}" alt="${p.name}" loading="lazy" onerror="this.style.display='none'">`;
      });
      imagesHtml += `</div>`;
    } else {
      imagesHtml = `<div style="background:var(--bg-tertiary);border-radius:12px;aspect-ratio:16/9;display:flex;align-items:center;justify-content:center;font-size:72px">🎸</div>`;
    }
...
$(document).ready(function () {
  ...
  $(document).on('click', '#modal-qty-dec', function () {
    const $qty = $('#modal-qty');
    const val = parseInt($qty.text());
    if (val > 1) $qty.text(val - 1);
  });

  $(document).on('click', '#modal-qty-inc', function () {
    const $qty = $('#modal-qty');
  ...
  if ($('#navbar-search').length) {
    $('#navbar-search').autocomplete({
      source: function (req, res) {
        $.get(`/api/products/autocomplete?q=${encodeURIComponent(req.term)}`, res);
      },
      minLength: 2,
      delay: 250,
      select: function (e, ui) {
        currentSearch = ui.item.value;
        currentPage = 1;
        if ($('#product-grid').length) loadProducts(1, false);
        else window.location.href = `/?q=${encodeURIComponent(ui.item.value)}`;
      },
    });
    ...
    $('#navbar-search').on('keydown', function (e) {
      if (e.key === 'Enter') {
        ...
        currentPage = 1;
        if ($('#product-grid').length) loadProducts(1, false);
        else window.location.href = `/?q=${encodeURIComponent(currentSearch)}`;
      }
    });
  }
    ...
    $('#checkout-form').validate({
      ...
      submitHandler: function () {
        ...
        $.ajax({
          ...
          success: function (data) {
            window.Cart.clear();
            showToast(`Order #${data.transaction.id} placed! 🎸`, 'success', 'Order Confirmed');
